...
func buildAWSConfig(wrap ConfigInfoWrapper, def ConfigDefinition, dbName, branch string) (FargateTaskDefinition, error) {
	...
	if len(stageInfo.Task) > 1 {
		...
		toReturn.Resources.Task.Properties.Family = fmt.Sprintf("%v-%v", taskStage, branch)
		toReturn.Resources.Task.Properties.CPU = taskStage.Name
		...
		toReturn.Description = fmt.Sprintf("Autogenerated cluster for the %v-%v fargate cluster", def.Name, branch)
		toReturn.Resources.ECSCluster.Properties.ClusterName = def.Name
		toReturn.Resources.Task.Properties.Family = fmt.Sprintf("%v-%v", def.Name, branch)
		toReturn.Resources.Task.Properties.CPU = stageInfo.CPU
		toReturn.Resources.Task.Properties.Memory = stageInfo.Memory
		toReturn.Resources.Task.Properties.TaskRoleArn = stageInfo.TaskRoleArn
		toReturn.Resources.Task.Properties.ExecutionRoleArn = stageInfo.ExecutionRoleArn
	}
	...
	for i := range taskStage.Services {
		cDef, err = buildContainerDefinition(taskStage.Services[i], branch, dbName)
		if err != nil {
...
func buildContainerDefinition(name, branch, dbName string) (ContainerDefinition, error) {
	var toReturn ContainerDefinition
	wrap, err := GetInfoFromDB(name, dbName)
	if err != nil {
		return toReturn, err
	}
